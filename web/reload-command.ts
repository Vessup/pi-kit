...
export function includeWebReloadCommand(commands: WebSlashCommand[]): WebSlashCommand[] {
	const visible = commands.filter((command) => command.name !== "web-reload");
	return visible.some((command) => command.name === "reload") ? visible : [WEB_RELOAD_COMMAND, ...visible];
}
