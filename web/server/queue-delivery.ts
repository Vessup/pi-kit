...
export function queueDeliveryFailureDisposition(previousAttempts: number): QueueDeliveryFailureDisposition {
	const attempts = Math.max(0, previousAttempts) + 1;
	if (attempts >= MAX_WEB_QUEUE_DELIVERY_ATTEMPTS) return { attempts, discard: true };
	return {
		attempts,
		discard: false,
...
export async function persistPreDeliveryTransition(options: {
	...
	try {
		...
		const disposition = queueDeliveryFailureDisposition(options.previousAttempts);
