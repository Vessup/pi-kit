...
const managedSessionStorePath = join(
...
function makeSessionRecord(
  ...
  const displayHistory = boundedWebHistory(
    kind === "saved" ? buildContextEntries(history as SessionEntry[]) : history,
  );
  ...
  const record = sessions.get(session.id) ?? {
    ...
    externalRequestTargets: new Map<
      string,
      Bun.ServerWebSocket<AgentSocketData>
    >(),
...
function broadcastSessionToAll(record: SessionRecord): void {
  if (record.catalogReady === false || sessions.get(record.id) !== record)
    return;
  broadcastToAll({
    type: "server.session",
...
function appendRecordHistory(record: SessionRecord, entry: unknown): void {
  ...
  record.historyReady = true;
  record.historyBytes =
    (record.historyBytes ?? webHistoryByteLength(record.history.slice(0, -1))) +
    ...
    return [
      {
        ...
        currentTool:
          typeof value.currentTool === "string" ? value.currentTool : undefined,
        queued:
  ...
  await runManagedRefresh(
      ...
      serializeManagedRefresh(record, async () => {
        ...
        try {
          ...
          const fileChanged = Boolean(
            newFile &&
              (!oldFile || sessionFileKey(newFile) !== sessionFileKey(oldFile)),
          );
          ...
          if (oldFile && oldFile !== record.file)
            sessionsByFile.delete(normalizePath(oldFile));
          if (record.file)
            sessionsByFile.set(normalizePath(record.file), record);
      ...
      }),
    {
      ...
      onBackgroundError: (error) =>
        console.error(`Could not refresh managed session ${record.id}:`, error),
    },
  );
}
...
async function createManagedSessionUnlocked(
  ...
  const existingRecord = resumed
    ? (sessions.get(resumed.session.id) ??
      sessionsByFile.get(normalizePath(resumed.file)))
    : undefined;
  ...
  try {
    ...
    for (const file of [sessionFile, record.file]) {
      if (file && sessionsByFile.get(normalizePath(file)) === record)
        sessionsByFile.delete(normalizePath(file));
    }
    if (existingRecord) {
...
async function createManagedSession(
  ...
  try {
    ...
    if (managedSessionStarts.get(key) === start)
      managedSessionStarts.delete(key);
  }
}
...
async function handleClientMessage(
    ...
    try {
      ...
      if (reload) {
        ...
        if (
          ...
          if (message.streamingBehavior === "steer")
            throw new Error("/compact must be queued or run while Pi is idle");
  ...
  if (message.type === "client.command") {
    const record =
      sessions.get(message.sessionId) ??
      sessionsByFile.get(normalizePath(message.sessionId));
    if (!record) {
      socket.send(
        JSON.stringify({
          type: "server.response",
...
async function routeCommandCore(
  ...
  if (
    ...
    if (record.status !== "idle")
      throw new Error("Wait for Pi to become idle before creating a worktree");
    ...
    const previousManagedWorktree =
      record.managedWorktree ??
      managedWorktreeFromEntries(parseSessionFile(previousFile)?.entries ?? []);
...
async function handleAgentMessage(
  ...
  if (message.type === "agent.response") {
    ...
    const record = Array.from(sessions.values()).find((candidate) =>
...
async function reconcileMissingSessionFiles(): Promise<void> {
  ...
  const candidates = [...sessions.values()].filter(
    (record) =>
      !missingSessionReconciliations.has(record) &&
...
async function deleteSession(sessionId: string): Promise<void> {
  let record =
    ...
    sessionsByFile.get(normalizePath(sessionId)) ??
    (() => {
      const scan = scanSavedSessions(sessionsDir).find(
        (item) =>
  ...
  if (record.file) {
    ...
    if (pendingStart) {
      ...
      record =
        sessions.get(sessionId) ??
        sessionsByFile.get(sessionFileKey(record.file)) ??
        record;
    }
  }
  const sessionFile = record.file;
...
async function handleApi(request: Request): Promise<Response> {
  ...
  if (request.method === "GET" && url.pathname === "/api/sessions") {
    ...
    for (const scan of scans) {
      const existing =
        sessions.get(scan.session.id) ??
        sessionsByFile.get(normalizePath(scan.file));
    ...
    for (const scan of scans) {
      ...
      merged.set(
        scan.session.file ? normalizePath(scan.session.file) : scan.session.id,
  ...
  if (request.method === "POST" && url.pathname === "/api/sessions") {
    ...
    const worktreeBranch =
      typeof body.worktreeBranch === "string" ? body.worktreeBranch.trim() : "";
  ...
  if (request.method === "POST" && url.pathname === "/api/sessions/resume") {
    ...
    try {
      ...
      return badRequest(
        error instanceof Error ? error.message : "Session file does not exist",
      );
    }
    const scan = parseSessionMetadataFile(file);
...
async function main(): Promise<void> {
  ...
  setTimeout(
    () =>
      void restoreManagedSessions().catch((error) => {
