...
export function validateWorktreeName(input: string): string {
  ...
  if (
    ...
    throw new Error(
      "Worktree name must be one safe path segment without '..', '/' or '\\'",
    );
  }
  if (!/^[A-Za-z0-9._-]+$/.test(name))
    throw new Error(
      "Worktree name may contain only letters, numbers, '.', '_' and '-'",
    );
  return name;
}
...
export function gitCommandTimeoutMs(args: readonly string[]): number {
  ...
  return args[commandIndex] === "worktree" &&
    (args[commandIndex + 1] === "add" || args[commandIndex + 1] === "remove")
...
function validateLocalBranchNameInput(input: string): string {
  ...
  if (branch.includes("\0"))
    throw new Error("Local branch name contains a NUL byte");
  if (branch.startsWith("refs/"))
    throw new Error(
      "Local branch must be a short branch name, not a refs/... path",
    );
  return branch;
}
...
export function validateLocalBranchName(input: string): string {
  ...
  if (result.status !== 0) {
    throw new Error(
      `Invalid local branch name ${JSON.stringify(branch)}: ${result.stderr?.trim() || result.error?.message || "git check-ref-format rejected it"}`,
    );
  }
  return branch;
}
...
async function validateLocalBranchNameAsync(input: string): Promise<string> {
  ...
  if (result.status !== 0) {
    throw new Error(
      `Invalid local branch name ${JSON.stringify(branch)}: ${result.stderr || "git check-ref-format rejected it"}`,
    );
  }
  return branch;
}
...
function spawnGitAsync(args: string[]): Promise<AsyncGitResult> {
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    child.once("close", (status, signal) => {
      ...
      if (timedOut || signal) {
        rejectPromise(
          new Error(`git ${args.join(" ")} did not finish within ${timeout}ms`),
        );
        return;
      }
...
export function validateGitVersion(versionOutput: string): void {
  const match = versionOutput
    .trim()
    .match(/^git version (\d+)\.(\d+)\.(\d+)(?:\D|$)/);
  if (!match)
    throw new Error(
...
async function ensureSupportedGitAsync(): Promise<void> {
  ...
  if (result.status !== 0)
    throw new Error(result.stderr || "Could not run git --version");
  validateGitVersion(result.stdout);
}
...
export type ManagedWorktree = Pick<
  CreatedWebWorktree,
  "path" | "repoRoot" | "name" | "branch" | "branchCreated"
>;
...
export function runWorktreeSetup(
  ...
  return new Promise((resolveSetup, rejectSetup) => {
    ...
    child.once("close", (code, signal) => {
      ...
      forceKillTimedOutGroup();
      if (timedOut)
        rejectSetup(new Error(`setup.sh timed out after ${timeoutMs}ms`));
      else if (code === 0) resolveSetup();
      else
        rejectSetup(
          new Error(
            `setup.sh exited with code ${code ?? `signal ${signal ?? "unknown"}`}`,
          ),
        );
    });
  });
...
export function managedWorktreeFromEntries(
  ...
  for (let index = entries.length - 1; index >= 0; index -= 1) {
    ...
    if (
      ...
      const name =
        typeof entry.data.name === "string" ? entry.data.name : basename(path);
      const branchCreated =
        typeof entry.data.branchCreated === "boolean"
          ? entry.data.branchCreated
          : name === branch;
...
export function sameManagedWorktreeIdentity(
  left: ManagedWorktree,
  right: ManagedWorktree,
): boolean {
  return (
...
export function inheritManagedBranchOwnership<T extends ManagedWorktree>(
  ...
  for (const candidate of existing) {
    if (
      candidate?.branchCreated === true &&
      sameManagedWorktreeIdentity(candidate, worktree)
    ) {
...
function parseRegisteredWorktrees(
  cwd: string,
): Array<{ path: string; head?: string; branch?: string; detached: boolean }> {
  ensureSupportedGit();
  const records: Array<{
    path: string;
    head?: string;
  ...
  let current:
    | { path: string; head?: string; branch?: string; detached: boolean }
    | undefined;
...
function primaryRepositoryRoot(cwd: string): string {
  ...
  if (!primary)
    throw new Error("Git repository has no registered primary worktree");
...
function resolveStartPoint(cwd: string, input: string): ResolvedStartPoint {
  ...
  if (value.includes("\0"))
    throw new Error("Worktree start point contains a NUL byte");
  if (value.startsWith("-"))
    throw new Error(`Invalid worktree start point: ${value}`);
  let commit: string;
  try {
    commit = gitOutput(cwd, [
      "rev-parse",
      "--verify",
      "--end-of-options",
      `${value}^{commit}`,
    ...
    throw new Error(
      `Worktree start point does not resolve to a commit: ${value} (${error instanceof Error ? error.message : String(error)})`,
    );
  }
  if (!/^[0-9a-f]{40,64}$/.test(commit))
    throw new Error(
      `Worktree start point resolved to an invalid commit: ${value}`,
    );
  ...
  const upstream =
    remoteMatch && remoteMatch[2] !== "HEAD"
...
function checkedOutBranchPath(cwd: string, branch: string): string | undefined {
  ...
  return parseRegisteredWorktrees(cwd).find((entry) => entry.branch === fullRef)
    ?.path;
}

/** Resolve and validate an existing worktree without changing its checkout or branch. */
export function inspectExistingWorktree(
  currentCwd: string,
  ...
  try {
    ...
    throw new Error(
      `Existing worktree path does not exist: ${requestedPath} (${error instanceof Error ? error.message : String(error)})`,
    );
  }
  if (!statSync(path).isDirectory())
    throw new Error(`Existing worktree path is not a directory: ${path}`);
  ...
  try {
    ...
    throw new Error(
      `Current CWD is not in a Git repository: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  try {
    ...
    throw new Error(
      `Target is not a Git worktree: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  if (currentGitDir !== targetGitDir)
    throw new Error(
      "Target worktree belongs to a different primary repository",
    );
  ...
  if (!registered)
    throw new Error(`Target is not a registered Git worktree: ${path}`);
  let ref: WorktreeRef;
  if (registered.branch?.startsWith("refs/heads/")) {
    ref = {
      kind: "branch",
      value: registered.branch.slice("refs/heads/".length),
    };
  } else if (
    registered.detached &&
    registered.head &&
    /^[0-9a-fA-F]{40,64}$/.test(registered.head)
  ) {
    ref = { kind: "detached", value: registered.head.toLowerCase() };
  } else {
    throw new Error(
      `Target worktree has no valid checked-out branch or detached HEAD: ${path}`,
    );
  }
...
export function currentWorktreeRef(cwd: string): WorktreeRef {
  ...
  if (symbolic.status === 0 && symbolic.stdout.trim())
    return { kind: "branch", value: symbolic.stdout.trim() };
  const head = gitOutput(cwd, ["rev-parse", "--verify", "HEAD"]);
  if (!/^[0-9a-fA-F]{40,64}$/.test(head))
    throw new Error(`Invalid detached HEAD: ${head}`);
 ...
 */
export function hasOtherSessionInWorktree(
  sessionsRoot: string,
  ...
  while (stack.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (
        ...
        samePath(file, currentSessionFile)
      )
        continue;
      try {
        ...
        if (
          ...
          pathIsWithin(header.cwd, worktreePath)
        )
          return true;
      } catch {
...
function verifiedManagedWorktreeLocation(
  ...
  if (basename(worktree.path) !== name)
    throw new Error(
      "Refusing to remove a worktree whose path and directory-name marker disagree",
    );
  ...
  if (dirname(path) !== managedRoot)
    throw new Error(`Refusing to remove worktree outside ${managedRoot}`);
  ...
  if (!samePath(commonDir, primaryGitDir))
    throw new Error(
      "Refusing to remove a worktree owned by another repository",
    );
  const topLevel = gitOutput(path, ["rev-parse", "--show-toplevel"]);
  if (!samePath(topLevel, path))
    throw new Error(
      "Refusing to remove a worktree whose Git root differs from its ownership marker",
    );
  return {
    path,
    repoRoot,
    name,
    branch,
    branchCreated: worktree.branchCreated === true,
  };
}
...
function verifiedManagedWorktree(worktree: ManagedWorktree): ManagedWorktree {
  ...
  if (actualRef.kind !== "branch" || actualRef.value !== verified.branch) {
    throw new Error(
      `Refusing to remove a worktree whose checked-out branch differs from its ownership marker (${verified.branch})`,
    );
  }
  return verified;
}

async function verifiedManagedWorktreeAsync(
  worktree: ManagedWorktree,
  ...
  if (symbolic.status !== 0 || symbolic.stdout !== branch) {
    throw new Error(
      `Refusing to remove a worktree whose checked-out branch differs from its ownership marker (${branch})`,
    );
  }
  return {
    path,
...
export function removeManagedWorktree(worktree: ManagedWorktree): {
  branchWarning?: string;
} {
  const verified = verifiedManagedWorktree(worktree);
  gitOutput(verified.repoRoot, [
    "worktree",
    "remove",
    "--force",
    verified.path,
...
export function rollbackWebWorktree(worktree: CreatedWebWorktree): void {
  ...
  if (dirty)
    throw new Error(
      `Refusing to roll back managed worktree with uncommitted or untracked changes: ${verified.path}`,
    );
  const branchAdvanced =
    verified.branchCreated &&
    worktree.initialCommit !== undefined &&
    gitOutput(verified.repoRoot, [
      "rev-parse",
      "--verify",
      `refs/heads/${verified.branch}^{commit}`,
    ]).toLowerCase() !== worktree.initialCommit;
  ...
  if (verified.branchCreated && !branchAdvanced) {
    if (worktree.initialCommit) {
      ...
      gitOutput(verified.repoRoot, [
        "update-ref",
        "-d",
        `refs/heads/${verified.branch}`,
        worktree.initialCommit,
      ]);
    } else {
      gitOutput(verified.repoRoot, ["branch", "-D", verified.branch]);
    }
...
export async function createWebWorktree(
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
      throw new Error(
        `Could not inspect worktree path ${path}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  if (pathExists) {
    if (options.startPoint !== undefined) {
      throw new Error(
        `Worktree ${path} already exists; omit the start point to reuse its current HEAD`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Worktree path already exists but cannot be reused: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    if (existing.ref.kind !== "branch")
      throw new Error(
        `Existing worktree at ${path} is detached; check out a branch before reusing it`,
      );
    if (requestedBranch && existing.ref.value !== branch) {
      throw new Error(
        `Existing worktree at ${path} has branch ${existing.ref.value}, not requested branch ${branch}`,
      );
    }
  ...
  const explicitStart =
    options.startPoint === undefined
      ? undefined
      : resolveStartPoint(repoRoot, options.startPoint);
  ...
  try {
    if (!branchExists) {
      ...
      if (explicitStart?.upstream) {
        gitOutput(repoRoot, [
          "branch",
          `--set-upstream-to=${explicitStart.upstream}`,
          branch,
        ]);
      }
    }
    ...
    try {
      registered = parseRegisteredWorktrees(repoRoot).some((entry) =>
        samePath(entry.path, path),
      );
    } catch (inspectionError) {
      cleanupErrors.push(
        `could not inspect partial worktree registration: ${inspectionError instanceof Error ? inspectionError.message : String(inspectionError)}`,
      );
    }
    ...
    if (registered)
      cleanupErrors.push(
        `worktree registration retained because ownership is ambiguous: ${path}`,
      );
    if (registered === false && branchProvisioned) {
      try {
        ...
        cleanupErrors.push(
          `could not remove created branch ${branch}: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`,
        );
      }
    }
  ...
  try {
    ...
    if (actualRef.kind !== "branch" || actualRef.value !== branch) {
      throw new Error(
        `Created worktree checked out ${actualRef.kind} ${actualRef.value}, expected branch ${branch}`,
      );
    }
    const actualHead = gitOutput(path, [
      "rev-parse",
      "--verify",
      "HEAD^{commit}",
    ]).toLowerCase();
    if (actualHead !== branchStart)
      throw new Error(
        `Created worktree HEAD ${actualHead} does not match requested start ${branchStart}`,
      );
    if (explicitStart?.upstream && !branchExists) {
      const upstreamResult = spawnGit([
        "-C",
        path,
        "rev-parse",
        "--abbrev-ref",
      ...
      if (actualUpstream !== explicitStart.upstream) {
        throw new Error(
          `Created branch upstream ${actualUpstream || "none"} does not match ${explicitStart.upstream}`,
        );
      }
    }
  } catch (error) {
    try {
      ...
      throw new Error(
        `${error instanceof Error ? error.message : String(error)}; worktree rollback failed: ${rollbackError instanceof Error ? rollbackError.message : String(rollbackError)}`,
      );
    }
    throw error;
  }
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
      throw new Error(
        `${error instanceof Error ? error.message : String(error)}; worktree retained at ${path} for inspection`,
      );
    }
  }
  if (setupInfo?.isFile()) {
