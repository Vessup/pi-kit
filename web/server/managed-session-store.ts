...
function readFiles(path: string): Set<string> {
	try {
		const parsed = JSON.parse(readFileSync(path, "utf8")) as Partial<ManagedSessionStoreFile>;
		if (parsed.version !== MANAGED_SESSION_STORE_VERSION || !Array.isArray(parsed.files) || parsed.files.some((file) => typeof file !== "string")) {
...
export class ManagedSessionStore {
	...
	#persist(): void {
		...
		try {
			writeFileSync(temporary, `${JSON.stringify({ version: MANAGED_SESSION_STORE_VERSION, files: this.list() }, null, 2)}\n`, { mode: 0o600 });
