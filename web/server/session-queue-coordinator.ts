...
export function createSessionQueueCoordinator(
  ...
  async function enqueueWebFollowUp(
    record: SessionRecord,
    ...
    await serializeQueueMutation(record, async () => {
      ...
      if (record.status === "idle" && record.agentRunning !== true)
        scheduleQueueSettleFallback(record);
    });
  }

  async function migratePersistedQueue(
    record: SessionRecord,
    oldId: string,
    newId: string,
  ): Promise<void> {
  ...
  function markWebQueueSnapshotDirty(record: SessionRecord): void {
    record.queueDirtyWorker ??= new DirtySnapshotRetryWorker({
      ...
      onError: (error) =>
        console.error(
          `Could not persist queue snapshot for ${record.id}:`,
          error,
        ),
    });
  ...
  function cancelQueueSettleFallback(record: SessionRecord): void {
    if (record.queueSettleFallbackTimer)
      clearTimeout(record.queueSettleFallbackTimer);
  ...
  function scheduleQueueSettleFallback(record: SessionRecord): void {
    ...
    record.queueSettleFallbackTimer = setTimeout(() => {
      record.queueSettleFallbackTimer = undefined;
      if (currentRecord(record.id) !== record || record.agentRunning === true)
        return;
  ...
  function broadcastQueueDelivery(
    record: SessionRecord,
    item: WebQueuedMessage,
    phase: "started" | "failed" | "uncertain",
    error?: string,
  ): void {
    broadcast(record.id, {
      type: "server.event",
  ...
  function sendSessionState(
    ...
    const payload: ServerSessionMessage = {
      type: "server.session",
      session: projectSession(record, true),
    };
    ...
    for (const uncertain of record.queue.filter(
      ...
      socket.send(
        JSON.stringify({
          ...
          sessionId: record.id,
          event: {
            type: "web_queue_delivery",
            phase: "uncertain",
            item: uncertain,
            error:
              "Delivery may already have been accepted; explicitly discard or confirm resubmission.",
          },
        } satisfies ServerEventMessage),
      );
    }
  ...
  async function flushWebQueueLocked(record: SessionRecord): Promise<void> {
    ...
    if (record.queue.some((queued) => queued.deliveryState === "delivering"))
      return;
    ...
    const transitioned = await persistPreDeliveryTransition({
      ...
      previousAttempts:
        record.queueTransitionAttempts?.itemId === item.id
          ? record.queueTransitionAttempts.count
          : 0,
      publishError: (error, attempts, exhausted) => {
        ...
        if (exhausted) {
          ...
          const liveItem = record.queue.find(
          ...
          broadcastQueueDelivery(
            ...
            liveItem,
            "uncertain",
            `Could not persist delivery transition after ${attempts} attempts: ${message}; explicitly discard or confirm resubmission.`,
          );
          broadcast(record.id, webQueueEvent(record));
      ...
      scheduleRetry: (delayMs) => {
        ...
        record.queueRetryTimer = setTimeout(() => {
          record.queueRetryTimer = undefined;
          if (
            currentRecord(record.id) === record &&
            record.queue[0]?.id === item?.id
          )
            void flushWebQueue(record);
        }, delayMs);
        record.queueRetryTimer.unref?.();
    ...
    try {
      ...
      if (error instanceof CommandDeliveryUncertainError) {
        ...
        broadcastQueueDelivery(
          ...
          item,
          "uncertain",
          `${message}; delivery may already have been accepted, so explicitly discard or confirm resubmission.`,
        );
        broadcast(record.id, webQueueEvent(record));
        return;
      }
      ...
      const previousAttempts =
        record.queueDeliveryAttempts?.itemId === item.id
      ...
      broadcastQueueDelivery(
        ...
        "failed",
        exhausted
          ? `${message} (discarded after ${attempts} attempts)`
          : `${message} (attempt ${attempts}; retrying)`,
      );
      if (exhausted) {
      ...
      console.error(
        `Could not deliver queued message for ${record.id}:`,
        error,
      );
    } finally {
    ...
    if (persistenceError) {
      ...
      if (uncertain) {
        ...
        broadcastQueueDelivery(
          ...
          uncertain,
          "uncertain",
          `Could not persist delivery failure: ${message}; explicitly discard or confirm resubmission.`,
        );
        broadcast(record.id, webQueueEvent(record));
      }
      return;
  ...
  async function routeQueueCommand(
    record: SessionRecord,
    command: Extract<
      ClientCommandMessage["command"],
      { type: "steer_queue_item" | "replace_queue" | "reconcile_queue" }
    >,
  ): Promise<unknown> {
    if (command.type === "steer_queue_item") {
      return serializeQueueMutation(record, async () => {
        if (record.queueDeliveryActive)
          throw new Error("Another queued message is already being delivered");
        const queued = record.queue.find((item) => item.id === command.itemId);
        if (!queued) throw new Error(`Unknown queue item ${command.itemId}`);
        if (queued.deliveryState === "delivering")
          throw new Error(
            `Queue item ${command.itemId} has uncertain delivery`,
          );
        ...
        if (
          ...
          throw new Error(
            `${queued.message.split(/\s/, 1)[0]} must remain queued until the current run settles`,
          );
        }
        ...
        try {
          await deliverCommand(record, {
            ...
            streamingBehavior:
              record.status === "working" ? "steer" : undefined,
          });
        } catch (error) {
          const message =
          ...
          try {
            ...
            const persistenceMessage =
              persistenceError instanceof Error
                ? persistenceError.message
                : String(persistenceError);
            const uncertain = record.queue.find(
              (candidate) => candidate.id === item.id,
            );
            if (uncertain) {
              uncertain.deliveryState = "delivering";
              broadcastQueueDelivery(
                ...
                uncertain,
                "uncertain",
                `Could not persist steer failure: ${persistenceMessage}; explicitly discard or confirm resubmission.`,
              );
              broadcast(record.id, webQueueEvent(record));
            }
            throw new Error(
              `${message}; could not persist queued-message recovery: ${persistenceMessage}`,
            );
          }
          throw error;
        } finally {
        ...
        record.queue = record.queue.filter(
          (candidate) => candidate.id !== item.id,
        );
        broadcast(record.id, webQueueEvent(record));
        markWebQueueSnapshotDirty(record);
      ...
      return serializeQueueMutation(record, async () => {
        ...
        for (const replacement of command.queue) {
          if (seenIds.has(replacement.id))
            throw new Error(`Duplicate queue item ${replacement.id}`);
          if (
            isWebReloadCommand(replacement.message) &&
            replacement.images?.length
          )
            throw new Error("/reload does not accept image attachments");
          if (
            parseWebCompactCommand(replacement.message) &&
            replacement.images?.length
          )
            throw new Error("/compact does not accept image attachments");
          ...
          if (
            ...
            throw new Error(
              `Queue item ${replacement.id} cannot set server-owned delivery state`,
            );
          }
          if (
            uncertain &&
            (replacement.message !== uncertain.message ||
              JSON.stringify(replacement.images ?? []) !==
                JSON.stringify(uncertain.images ?? []))
          ) {
            throw new Error(
              `Uncertain queue item ${uncertain.id} requires explicit discard or resubmit`,
            );
          }
        }
        for (const uncertain of uncertainById.values()) {
          if (!seenIds.has(uncertain.id))
            throw new Error(
              `Uncertain queue item ${uncertain.id} requires explicit discard or resubmit`,
            );
        }
    ...
    if (command.type === "reconcile_queue") {
      return serializeQueueMutation(record, async () => {
        const item = record.queue.find(
          (queued) => queued.id === command.itemId,
        );
        if (!item || item.deliveryState !== "delivering")
          throw new Error(`Queue item ${command.itemId} is not uncertain`);
        await transactionalQueueMutation({
          get: () => record.queue,
          set: (queue) => setWebQueueState(record, queue),
          clone: cloneWebQueue,
          mutate: (queue) => {
            ...
            if (index < 0)
              throw new Error(
                `Queue item ${item.id} disappeared during reconciliation`,
              );
