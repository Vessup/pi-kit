export type SerializedMutationState = {
...
export function serializeQueueMutation<T>(
  state: SerializedMutationState,
  task: () => Promise<T>,
): Promise<T> {
  if (state.queueMutationsQuiesced)
    return Promise.reject(new Error("Queue mutations are quiesced"));
  const run = (state.queueMutationTail ?? Promise.resolve())
    .catch(() => undefined)
    .then(task);
  state.queueMutationTail = run.then(
    () => undefined,
    () => undefined,
  );
  return run;
...
export async function quiesceQueueMutations(
  state: SerializedMutationState,
): Promise<void> {
