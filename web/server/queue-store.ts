...
type QueueStoreFile = {
  version: 2;
  queues: Record<string, WebQueuedMessage[]>;
};
...
function parseQueuedMessage(value: unknown): WebQueuedMessage | undefined {
  if (
    ...
    return undefined;
  if (value.deliveryState !== undefined && value.deliveryState !== "delivering")
    return undefined;
...
export function readQueueStore(path: string): Map<string, WebQueuedMessage[]> {
  ...
  if (
    !isRecord(parsed) ||
    (parsed.version !== 1 && parsed.version !== QUEUE_STORE_VERSION) ||
    !isRecord(parsed.queues)
  ) {
    throw invalidQueueStore(
      path,
      "expected a supported version and queues object",
    );
  }
  ...
  for (const [sessionId, rawQueue] of Object.entries(parsed.queues)) {
    if (!Array.isArray(rawQueue))
      throw invalidQueueStore(
        path,
        `queue ${JSON.stringify(sessionId)} is not an array`,
      );
    ...
    for (const item of rawQueue) {
      ...
      if (!parsedItem)
        throw invalidQueueStore(
          path,
          `queue ${JSON.stringify(sessionId)} contains an invalid item`,
        );
...
function cloneQueues(
  queues: ReadonlyMap<string, readonly WebQueuedMessage[]>,
): Map<string, WebQueuedMessage[]> {
  return new Map(
    Array.from(queues, ([sessionId, queue]) => [
      sessionId,
      queue.map((item) => ({
        ...item,
...
function snapshotQueueStore(
  queues: ReadonlyMap<string, readonly WebQueuedMessage[]>,
): QueueStoreFile {
  const serialized: QueueStoreFile = {
...
function restoreQueues(
  target: Map<string, WebQueuedMessage[]>,
  snapshot: ReadonlyMap<string, readonly WebQueuedMessage[]>,
): void {
  target.clear();
  for (const [sessionId, queue] of cloneQueues(snapshot))
    target.set(sessionId, queue);
}

/** Atomically replace the queue store without blocking the server on filesystem I/O. */
export async function writeQueueStore(
  path: string,
  queues: ReadonlyMap<string, readonly WebQueuedMessage[]>,
): Promise<void> {
  ...
  try {
    await writeFile(tempPath, JSON.stringify(snapshotQueueStore(queues)), {
      mode: 0o600,
    });
...
export class CoalescedQueueStoreWriter {
  ...
  constructor(
    ...
    private readonly snapshotWriter?: QueueSnapshotWriter,
  ) {}
  ...
  mutate(
    queues: Map<string, WebQueuedMessage[]>,
    mutation: (queues: Map<string, WebQueuedMessage[]>) => void,
  ): Promise<void> {
  ...
  private enqueue(snapshot: QueueStoreFile): Promise<void> {
    ...
    const promise = new Promise<void>((resolve, reject) =>
      this.pending.push({ revision, snapshot, resolve, reject }),
    );
    if (!this.running) {
      this.running = true;
    ...
    while (this.pending.length > 0) {
      ...
      try {
        ...
        const completed = this.pending.filter(
          (write) => write.revision <= target.revision,
        );
        ...
        const failed = this.pending.filter(
          (write) => write.revision <= target.revision,
        );
        this.pending = this.pending.filter(
          (write) => write.revision > target.revision,
