export type SessionStatus = "idle" | "working" | "offline" | "starting" | "error";
...
export type WebSubagentTranscriptItem = { timestamp: number; role: string; text: string };

export type WebSubagent = {
	id: string;
	status: "creating" | "working" | "completed" | "failed" | "terminating" | "terminated";
	model: string;
	effort: string;
	turns: number;
	currentTool?: string;
...
function trimWebSubagentTranscript(items: WebSubagentTranscriptItem[]): WebSubagentTranscriptItem[] {
...
export function mergeWebSubagentUpdates(
	...
	return updates.map((update) => {
		...
		else if (update.transcriptDelta) transcript = [...(transcript ?? []), ...update.transcriptDelta];
		if (transcript) transcript = trimWebSubagentTranscript(transcript);

		let streamingText = prior?.streamingText;
		if (update.streamingTextReset) streamingText = update.streamingTextDelta || undefined;
		else if (update.streamingTextDelta !== undefined) streamingText = `${streamingText ?? ""}${update.streamingTextDelta}` || undefined;
...
export type WebCompaction = { reason: "manual" | "threshold" | "overflow"; startedAt: number };
...
export function orderWebSessions(sessions: WebSession[], customOrder: readonly string[]): WebSession[] {
	...
	return defaultOrder.sort((a, b) => {
		...
		if (aRank === undefined && bRank === undefined) return compareWebSessions(a, b);
...
export function moveWebSession(
	...
	return moveWebSessionRelative(sessions, customOrder, sourceId, { beforeId: targetId });
}
...
export function moveWebSessionRelative(
	...
	const ids = orderWebSessions(sessions, customOrder).map((session) => session.id);
...
export type WebSessionOptions = { models: WebModelOption[]; thinkingLevels: string[]; commands: WebSlashCommand[] };
...
export function moveWebQueuedMessage(
	...
	const next = queue.map((item) => ({ ...item, images: item.images?.map((image) => ({ ...image })) }));
	const sourceIndex = next.findIndex((item) => item.id === sourceId);
	if (sourceIndex < 0) return next;
	const [source] = next.splice(sourceIndex, 1);
	if (!source) return next;
	if (placement.beforeId && placement.beforeId !== sourceId) {
		const targetIndex = next.findIndex((item) => item.id === placement.beforeId);
