...
export function compactionSummaryHistoryEntry(entry: unknown, fallbackIndex = 0): unknown | undefined {
	if (!isRecord(entry) || entry.type !== "compaction" || typeof entry.summary !== "string") return undefined;
...
export function messagesToWebHistory(messages: readonly unknown[]): unknown[] {
	return boundedWebHistory(messages.flatMap<unknown>((message, index) => {
		...
		if (message.role === "compactionSummary" && typeof message.summary === "string") {
			return [{
				...
				parentId: null,
				timestamp: typeof message.timestamp === "number" ? new Date(message.timestamp).toISOString() : new Date().toISOString(),
				summary: message.summary,
			}];
		}
		if (message.role === "branchSummary" || (message.role === "custom" && message.display !== true)) return [];
		if (!["user", "assistant", "toolResult", "bashExecution", "custom"].includes(message.role)) return [];
		return [{
			type: "message",
