...
export function parseWebCompactCommand(text: string): WebCompactCommand | undefined {
...
export function includeWebCompactCommand(commands: WebSlashCommand[]): WebSlashCommand[] {
	return commands.some((command) => command.name === "compact") ? commands : [WEB_COMPACT_COMMAND, ...commands];
}
