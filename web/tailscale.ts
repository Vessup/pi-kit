...
export function parseTailscaleWebSettings(
  value: unknown,
): TailscaleWebSettings {
  const file =
    value && typeof value === "object" ? (value as WebSettingsFile) : {};
  const serviceName =
    typeof file.web?.tailscale?.serviceName === "string"
...
export async function readTailscaleWebSettings(
  settingsPath: string,
): Promise<TailscaleWebSettings> {
  try {
    return parseTailscaleWebSettings(
...
export async function findTailscaleCli(
...
export async function defaultTailscaleRunner(
  command: string,
  args: readonly string[],
): Promise<{ stdout: string; stderr: string }> {
...
export async function disableTailscaleServe(options: {
  ...
  try {
    await run(command, [
      "serve",
      "--yes",
      ...(options.settings.serviceName
        ? [`--service=svc:${options.settings.serviceName}`]
        : []),
...
export async function ensureTailscaleServe(options: {
  ...
  if (!command) {
    return {
      ...
      available: false,
      published: false,
      error: options.settings.enabled
        ? "Tailscale is enabled for Pi web, but the Tailscale CLI is not installed."
        : undefined,
    };
  }
  ...
  const run = options.run ?? defaultTailscaleRunner;
  try {
    const status = parseCliStatus(
      (await run(command, ["status", "--json"])).stdout,
    );
    ...
    if (!dnsName)
      throw new Error(
        "Tailscale did not report a MagicDNS name for this device",
      );
    await run(command, [
      "serve",
      "--bg",
...
function isSameServeRoute(
  left: TailscaleWebSettings,
  right: TailscaleWebSettings,
): boolean {
  return (
    left.httpsPort === right.httpsPort && left.serviceName === right.serviceName
  );
}
...
export async function replaceTailscaleServe(options: {
  ...
  const routeChanged = !isSameServeRoute(
  ...
  if (!status.published) {
    ...
    if (!options.currentSettings.enabled || routeChanged) {
      const cleanup = await disableTailscaleServe({
        ...shared,
        settings: options.nextSettings,
      });
      if (cleanup.error) {
        return {
          ...status,
          error: `${status.error ?? "Replacement route failed"}; replacement cleanup failed: ${cleanup.error}`,
        };
      }
    }
    return status;
  ...
  const removed = await disableTailscaleServe({
    ...shared,
