    ...
    if (escaped) {
      ...
      if (
        ...
        if (jsonEscape !== undefined) current += jsonEscape;
          ...
          quote === '"' &&
          (hasWindowsPathPrefix(current) || startsRootRelativeWindowsPath) &&
          char !== "\\" &&
          char !== '"'
        )
          current += `\\${char}`;
      ...
      char === "\\" &&
      (quote === '"' || (!quote && /\s/.test(source[index + 1] ?? "")))
    ) {
      escaped = true;
    } else if (quote) {
...
export const WORKTREE_USAGE =
  "Usage: /worktree <name> [--repo <path>] [--branch <local-branch>] [--start-point <ref>] | /worktree --existing <worktree-path>";
...
export function parseWorktreeCommandArgs(input: string): WorktreeCommandArgs {
  ...
  for (let index = 0; index < words.length; index += 1) {
    ...
    if (word === "--repo" || word === "-C") {
      ...
      if (branch !== undefined)
        throw new Error("Duplicate /worktree branch option");
      ...
      if (startPoint !== undefined)
        throw new Error("Duplicate /worktree start-point option");
      ...
      if (!startPoint)
        throw new Error("--start-point requires a Git ref or commit");
    } else if (word === "--existing") {
      if (existing !== undefined)
        throw new Error("Duplicate /worktree existing option");
  ...
  if (existing && (name || repository || branch || startPoint))
    throw new Error(WORKTREE_USAGE);
...
export function formatWorktreeCreateCommandArgs(input: {
  ...
  return [
    ...
    input.startPoint
      ? `--start-point ${JSON.stringify(input.startPoint)}`
      : undefined,
  ]
...
export function parseWorktreeInvocation(
  text: string,
): WorktreeCommandArgs | undefined {
