...
export function anchoredPopoverPosition(options: {
  ...
  const maxLeft = Math.max(minLeft, viewport.offsetLeft + viewport.width - panelWidth - margin);
  const maxTop = Math.max(minTop, viewport.offsetTop + viewport.height - panelHeight - margin);
  const desiredLeft = align === "start" ? anchor.left : anchor.right - panelWidth;
  const below = anchor.bottom + gap;
  const desiredTop = below + panelHeight <= viewport.offsetTop + viewport.height - margin
    ? below
