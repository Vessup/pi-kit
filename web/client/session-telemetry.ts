...
function preserveSubagentTelemetry(
  previous: WebSession["subagents"],
  next: WebSession,
): WebSession["subagents"] {
  if (next.subagents === undefined)
    return next.status === "offline" || next.source === "saved" ? [] : previous;
 ...
 */
export function preserveSessionTelemetry(
  ...
  return {
    ...
    usage:
      hasUsage(next.usage) || !hasUsage(previous.usage)
        ? next.usage
        : previous.usage,
...
export function preserveSessionsTelemetry(
  previous: readonly WebSession[],
  next: readonly WebSession[],
): WebSession[] {
  const priorById = new Map(previous.map((session) => [session.id, session]));
  return next.map((session) =>
    preserveSessionTelemetry(priorById.get(session.id), session),
  );
}
