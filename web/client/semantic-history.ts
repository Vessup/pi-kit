...
export function semanticEntryIdentity(
  entry: SemanticEntry,
): string | undefined {
  ...
  if (entry.id && !entry.id.startsWith("optimistic-"))
    return `entry:${entry.id}`;
  return undefined;
}

export function preserveSemanticEntryKeys(
  ...
  for (const entry of previous) {
    ...
    if (identity && entry.id && !entry.id.startsWith("optimistic-"))
      previousIds.set(identity, entry.id);
  }
...
export function mergeSemanticHistory(
  previous: SemanticEntry[],
  incoming: SemanticEntry[],
): SemanticEntry[] {
...
export function semanticHistoriesEqual(
  previous: SemanticEntry[],
  incoming: SemanticEntry[],
): boolean {
  ...
  return previous.every((entry, index) => {
    ...
    if (!next || semanticEntryIdentity(entry) !== semanticEntryIdentity(next))
      return false;
    try {
      return (
        JSON.stringify({ type: entry.type, message: entry.message }) ===
        JSON.stringify({ type: next.type, message: next.message })
      );
    } catch {
      return false;
    }
