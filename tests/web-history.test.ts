...
test("web history reserves space for the compaction summary", () => {
  ...
  expect(history.map((entry) => (entry as { id?: string }).id)).toEqual([
    "web-compaction-compact",
    "two",
  ]);
});

test("web history remains byte bounded and truncates oversized content", () => {
  const history = boundedWebHistory([message("large", "x".repeat(400_000))], {
    maxBytes: 400_000,
  });
...
test("oversized history images become explicit omission markers", () => {
  ...
  expect(JSON.stringify(history)).toContain(
    "omitted an oversized image/png attachment",
  );
});
