...
} from "../web/compact-command.ts";
import {
  includeWebReloadCommand,
  isWebReloadCommand,
} from "../web/reload-command.ts";
import {
  expandSlashCommand,
  isSkillSlashCommand,
} from "../web/slash-commands.ts";
...
test("native web prompts expand Pi prompt templates with quoted arguments and defaults", async () => {
  ...
  expect(
    await expandSlashCommand(commands, '/address-pr "owner/repo#42"'),
...
test("prompt arguments preserve empty and escaped quoted values", async () => {
  ...
  const commands = [
    { name: "args", source: "prompt" as const, sourceInfo: { path } },
  ];

  expect(
    await expandSlashCommand(commands, '/args "" second "say \\"hi\\""'),
  ).toBe(
    'First: []\nSecond: [second]\nThird: [say "hi"]\nAll: [ second say "hi"]',
  );
  expect(
    await expandSlashCommand(
      commands,
      `/args C:\\tmp\\file '\\d+' "say \\"hi\\""`,
    ),
  ).toBe(
    'First: [C:\\tmp\\file]\nSecond: [\\d+]\nThird: [say "hi"]\nAll: [C:\\tmp\\file \\d+ say "hi"]',
  );
  await expect(
    expandSlashCommand(commands, '/args "unterminated'),
...
test("web compact routing accepts optional instructions without matching prose", () => {
  ...
  expect(parseWebCompactCommand("/compact preserve file names ")).toEqual({
    customInstructions: "preserve file names",
  });
...
test("the web slash menu exposes control commands across stale native metadata", () => {
  const commands = includeWebCompactCommand(
    includeWebReloadCommand([
      {
        name: "web-reload",
  ...
  expect(
    includeWebReloadCommand(commands).filter(
      (command) => command.name === "reload",
    ),
  ).toHaveLength(1);
  expect(
    includeWebCompactCommand(commands).filter(
