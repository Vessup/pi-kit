...
} from "../web/server/queue-mutation.ts";
...
test("session queue mutations are deterministic and serialized", async () => {
  ...
  const first = serializeQueueMutation(state, async () => {
    order.push("first:start");
    await gate;
    order.push("first:end");
  });
  const second = serializeQueueMutation(state, async () => {
    order.push("second");
  });
  await Bun.sleep(0);
...
test("queue mutation quiescing drains admitted work and rejects late intake", async () => {
  ...
  let drained = false;
  const quiesced = quiesceQueueMutations(state).then(() => {
    drained = true;
  });
  await Bun.sleep(0);
  assert.equal(drained, false);
  await assert.rejects(
    serializeQueueMutation(state, async () => undefined),
    /quiesced/,
  );
  release();
...
test("failed persistence rolls back only its serialized mutation", async () => {
  ...
  const failedWrite = new Promise<void>((_, reject) => {
    rejectWrite = reject;
  });
  const failed = serializeQueueMutation(state, () =>
    transactionalQueueMutation({
      get: () => queue,
      set: (next) => {
        queue = next;
  ...
  const later = serializeQueueMutation(state, () =>
    transactionalQueueMutation({
      get: () => queue,
      set: (next) => {
        queue = next;
...
test("background accepted-removal retry does not retain the mutation tail", async () => {
  ...
  const accepted = serializeQueueMutation(state, async () => {
    void retry;
  });
  await accepted;
  let laterRan = false;
  await serializeQueueMutation(state, async () => {
    laterRan = true;
  });
