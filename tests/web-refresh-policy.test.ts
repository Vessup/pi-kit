...
import { runManagedRefresh, serializeManagedRefresh } from "../web/server/refresh-policy.ts";
...
test("foreground managed refresh propagates migration failures", async () => {
	...
	await assert.rejects(runManagedRefresh(async () => { throw failure; }), failure);
});
...
test("complete managed refreshes serialize per record across foreground and background callers", async () => {
	...
	const background = serializeManagedRefresh(state, async () => { order.push("background:refresh"); });
	await Bun.sleep(0);
	assert.deepEqual(order, ["foreground:get-state"]);
	release();
	await Promise.all([foreground, background]);
	assert.deepEqual(order, ["foreground:get-state", "foreground:rekey", "background:refresh"]);
});

test("failed foreground refresh does not poison serialized background refresh", async () => {
	const state: { managedRefreshTail?: Promise<void> } = {};
	const failure = new Error("migration failed");
	const foreground = serializeManagedRefresh(state, async () => { throw failure; });
	const observed: string[] = [];
	const background = serializeManagedRefresh(state, async () => { observed.push("resumed"); });
...
test("background managed refresh suppresses failures only when explicitly requested", async () => {
	...
	await runManagedRefresh(async () => { throw failure; }, {
		suppressErrors: true,
