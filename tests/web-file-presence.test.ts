...
import {
  errorConfirmsMissingPath,
  isConfirmedMissingPath,
} from "../web/server/file-presence.ts";
...
test("missing-path reconciliation requires an explicit ENOENT", () => {
  ...
  expect(
    errorConfirmsMissingPath(
      Object.assign(new Error("forbidden"), { code: "EACCES" }),
    ),
  ).toBe(false);
  expect(
    errorConfirmsMissingPath(
  ...
  expect(
    errorConfirmsMissingPath(new Error("unknown filesystem failure")),
  ).toBe(false);
});
