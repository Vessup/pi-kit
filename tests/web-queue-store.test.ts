...
test("web follow-up queues survive daemon restarts with attachments", async () => {
	...
	await writeQueueStore(path, new Map([[
		"session-1",
		[{ id: "queued-1", message: "do this next", images: [{ type: "image", data: "aGVsbG8=", mimeType: "image/png", name: "image.png" }] }],
	]]));
...
test("daemon restart preserves an in-flight item as blocked instead of redelivering or erasing it", async () => {
	...
	const item = { id: "queued-crash", message: "perform once", deliveryState: "delivering" as const };
...
test("queue store corruption is propagated instead of dropping malformed entries", async () => {
	tempDir = await mkdtemp(join(tmpdir(), "pi-kit-queue-store-"));
	const path = join(tempDir, "queues.json");
	await Bun.write(path, JSON.stringify({ version: 1, queues: { session: [{ id: 7, message: "bad" }] } }));
...
test("shared queue-map mutations roll back globally before a later session snapshots", async () => {
	...
	const firstWriteStarted = new Promise<void>((resolve) => { markStarted = resolve; });
...
test("coalesced queue writes durably preserve the newest snapshot", async () => {
	...
	const first = writer.write(new Map([["session", [{ id: "one", message: "first" }]]]));
	const second = writer.write(new Map([["session", [
		{ id: "one", message: "first" },
		{ id: "two", message: "newest", images: [{ type: "image", data: "aGVsbG8=", mimeType: "image/png" }] },
	]]]));
	await Promise.all([first, second]);
	expect(readQueueStore(path).get("session")).toEqual([
		{ id: "one", message: "first" },
		{ id: "two", message: "newest", images: [{ type: "image", data: "aGVsbG8=", mimeType: "image/png", name: undefined }] },
	]);
});
