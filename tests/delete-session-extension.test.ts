...
test("delete-session rechecks managed worktree ownership after confirmation", async () => {
	...
	try {
		...
		await writeFile(sessionFile, `${JSON.stringify({ type: "session", cwd: worktreePath })}\n`);
		...
		await handler("", {
			...
			sessionManager: {
				...
				getEntries: () => [{
					...
					data: { path: worktreePath, repoRoot: directory, name: "managed-worktree", branch: "topic", branchCreated: true },
				}],
			},
			ui: {
				confirm: async (_title: string, message: string) => {
					expect(message).toContain("managed worktree");
					await writeFile(join(directory, "other.jsonl"), `${JSON.stringify({ type: "session", cwd: worktreePath })}\n`);
					return true;
				},
		...
		expect(notifications.some((message) => message.includes("worktree cleanup failed"))).toBe(false);
