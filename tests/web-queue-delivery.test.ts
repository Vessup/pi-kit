...
test("queued delivery failures back off and reach a bounded discard disposition", () => {
	...
	assert.deepEqual(final, { attempts: MAX_WEB_QUEUE_DELIVERY_ATTEMPTS, discard: true });
});
...
test("pre-delivery persistence publishes bounded retries and gates exactly-once delivery", async () => {
	...
	const run = async (): Promise<void> => {
		const transitioned = await persistPreDeliveryTransition({
			persist: async () => { if (++writes < 3) throw new Error("disk unavailable"); },
			previousAttempts: attempts,
			publishError: (_error, next, exhausted) => { attempts = next; errors.push({ attempts: next, exhausted }); },
			scheduleRetry: () => { scheduled.push(run); },
		});
	...
	assert.deepEqual(errors, [{ attempts: 1, exhausted: false }, { attempts: 2, exhausted: false }]);
});
...
test("pre-delivery persistence retry is bounded", async () => {
	...
	const run = async (): Promise<void> => {
		await persistPreDeliveryTransition({
			persist: async () => { throw new Error("disk unavailable"); }, previousAttempts: attempts,
			publishError: (_error, next, final) => { attempts = next; exhausted.push(final); },
			scheduleRetry: () => { scheduled.push(run); },
		});
	};
	await run(); await scheduled.shift()!(); await scheduled.shift()!();
...
test("dirty snapshot worker keeps one retry loop during permanent failure", async () => {
	...
	const worker = new DirtySnapshotRetryWorker({
		persist: async () => { attempts++; throw new Error("disk unavailable"); },
...
test("dirty snapshot worker coalesces multiple accepted removals without losing later state", async () => {
	...
	let writes = 0;
	const worker = new DirtySnapshotRetryWorker({ persist: async () => {
...
test("dirty snapshot worker restarts when markDirty interleaves with finalization", async () => {
	...
	let injected = false;
	worker = new DirtySnapshotRetryWorker({ persist: async () => {
		writes++;
		if (!injected) {
			injected = true;
...
test("dirty snapshot worker performs one bounded final shutdown snapshot", async () => {
	...
	const finalWrite = new Promise<void>((resolve) => { releaseFinal = resolve; });
	const worker = new DirtySnapshotRetryWorker({
		persist: async () => {
			attempts++;
	...
	const ordered = new DirtySnapshotRetryWorker({ persist: async () => {
	...
	await Bun.sleep(0);
	assert.equal(orderedAttempts, 1, "final persist must wait for the in-flight persist");
	releaseOld();
	assert.equal(await orderedDrain, true);
	assert.deepEqual(snapshots, ["old", "final"]);

	const wedged = new DirtySnapshotRetryWorker({ persist: () => new Promise<void>(() => undefined) });
...
test("dirty snapshot worker cancellation drains an in-flight old-identity write", async () => {
	...
	await Bun.sleep(0);
	let drained = false;
	const drain = worker.cancelAndDrain().then(() => { drained = true; });
	identity = "new";
	await Bun.sleep(0);
...
test("dirty snapshot worker cancellation interrupts a custom delay", async () => {
	...
	const worker = new DirtySnapshotRetryWorker({
		persist: async () => { attempts++; throw new Error("permanent"); },
		delay: () => { delayStarted(); return new Promise<void>(() => undefined); },
	});
	worker.markDirty();
	await started;
	worker.cancel();
