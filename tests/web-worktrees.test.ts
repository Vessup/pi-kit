...
test("checkout-producing Git commands use the long-running operation bound", () => {
  ...
  expect(
    gitCommandTimeoutMs(["-C", "/repo", "worktree", "remove", "/checkout"]),
  ).toBe(10 * 60_000);
  expect(
    gitCommandTimeoutMs(["-C", "/repo", "worktree", "list", "--porcelain"]),
  ).toBe(30_000);
  expect(gitCommandTimeoutMs(["check-ref-format", "--branch", "topic"])).toBe(
    30_000,
  );
});
...
test("web worktrees are created under the primary repository .pi directory", async () => {
  ...
  expect(await realpath(result.path)).toBe(
    await realpath(join(repository, ".pi", "worktrees", "feature-one")),
  );
  ...
  await expect(
    createWebWorktree(repository, "feature-one", { startPoint: "HEAD" }),
  ).rejects.toThrow("omit the start point");
  await expect(
    createWebWorktree(repository, "feature-one", { startPoint: "not-a-ref" }),
  ).rejects.toThrow("omit the start point");
  await expect(
    createWebWorktree(repository, "feature-one", {
      branch: "different-branch",
    }),
  ).rejects.toThrow("not requested branch different-branch");
  ...
  expect(
    (await Bun.$`git -C ${result.path} branch --show-current`.text()).trim(),
  ).toBe("feature-one");
  const stalePath = join(repository, ".pi", "worktrees", "stale");
  await mkdir(stalePath, { recursive: true });
  await expect(createWebWorktree(repository, "stale")).rejects.toThrow(
    "already exists but cannot be reused",
  );
...
test("managed directory, local branch, remote start point, and upstream are independent", async () => {
  ...
  const remoteHead = (
    await Bun.$`git -C ${repository} rev-parse origin/tembo/cancel-builds`.text()
  ).trim();
  ...
  expect(
    (await Bun.$`git -C ${result.path} rev-parse HEAD`.text()).trim(),
  ).toBe(remoteHead);
  expect(
    (
      await Bun.$`git -C ${result.path} rev-parse --abbrev-ref --symbolic-full-name '@{upstream}'`.text()
    ).trim(),
  ).toBe("origin/tembo/cancel-builds");
  expect(
    (await Bun.$`git -C ${repository} branch --list pr-30`.text()).trim(),
  ).toBe("");
  ...
  expect(
    (
      await Bun.$`git -C ${repository} branch --list tembo/cancel-builds`.text()
    ).trim(),
  ).toBe("");
});
...
test("an unused existing local branch is reused and preserved by rollback", async () => {
  ...
  const branchHead = (
    await Bun.$`git -C ${repository} rev-parse reusable/topic`.text()
  ).trim();
  await writeFile(join(repository, "README.md"), "main moved\n");
  await Bun.$`git -C ${repository} commit -am main-moved -q`;

  const result = await createWebWorktree(repository, "local-copy", {
  ...
  await expect(stat(result.path)).rejects.toThrow();
  expect(
    (await Bun.$`git -C ${repository} rev-parse reusable/topic`.text()).trim(),
  ).toBe(branchHead);
  await expect(
    createWebWorktree(repository, "conflicting-start", {
      branch: "reusable/topic",
      startPoint: "HEAD",
    }),
...
test("invalid branches and missing start points fail before creating resources", async () => {
  ...
  await expect(
    createWebWorktree(repository, "missing", {
      branch: "owner/missing",
      startPoint: "origin/does-not-exist",
    }),
  ).rejects.toThrow("does not resolve to a commit");
  await expect(
    createWebWorktree(repository, "invalid", { branch: "bad..branch" }),
  ).rejects.toThrow("Invalid local branch name");
  await expect(
    createWebWorktree(repository, "option", {
      branch: "owner/option",
      startPoint: "--all",
    }),
  ).rejects.toThrow("Invalid worktree start point");
  for (const name of ["missing", "invalid", "option"]) {
    await expect(
      stat(join(repository, ".pi", "worktrees", name)),
    ).rejects.toThrow();
  }
  expect(
    (
      await Bun.$`git -C ${repository} branch --list owner/missing`.text()
    ).trim(),
  ).toBe("");
});
...
test("existing registered worktrees are resolved without modifying their branch", async () => {
  ...
  const beforeHead = (
    await Bun.$`git -C ${worktree} rev-parse HEAD`.text()
  ).trim();
  ...
  expect(currentWorktreeRef(worktree)).toEqual({
    kind: "branch",
...
test("existing detached worktrees retain their exact HEAD", async () => {
  ...
  expect(inspectExistingWorktree(repository, worktree).ref).toEqual({
    kind: "detached",
    value: head,
  });
  expect(currentWorktreeRef(worktree)).toEqual({
...
test("existing targets reject missing paths, unregistered checkouts, and other repositories", async () => {
  ...
  expect(() =>
    inspectExistingWorktree(repository, join(directory, "missing")),
  ).toThrow("does not exist");
  const nested = join(repository, "nested");
  await mkdir(nested);
  expect(() => inspectExistingWorktree(repository, nested)).toThrow(
    "not a registered Git worktree",
  );
  expect(() => inspectExistingWorktree(repository, other)).toThrow(
...
test("executable worktree setup scripts run through their shebang", async () => {
  ...
  expect(await readFile(join(result.path, "setup-runtime.txt"), "utf8")).toBe(
    "bash-shebang",
  );
});
...
test("failed setup retains the new checkout and branch for inspection", async () => {
  directory = await mkdtemp(
    join(tmpdir(), "pi-kit-web-worktree-setup-failure-"),
  );
  const repository = join(directory, "repo");
  await Bun.$`git init -q ${repository}`;
  await Bun.$`git -C ${repository} config user.name test`;
  ...
  await expect(createWebWorktree(repository, "setup-fails")).rejects.toThrow(
    "worktree retained",
  );
  expect(
    (
      await stat(join(repository, ".pi", "worktrees", "setup-fails"))
    ).isDirectory(),
  ).toBe(true);
  expect(
    (
      await Bun.$`git -C ${repository} branch --list --format=%\(refname:short\) setup-fails`.text()
    ).trim(),
  ).toBe("setup-fails");
});
...
test("worktree setup is terminated after its bounded timeout", async () => {
  ...
  await expect(runWorktreeSetup(script, [], directory, 50)).rejects.toThrow(
...
test("worktree setup force-kills descendants after a timed-out leader exits", async () => {
  ...
  directory = await mkdtemp(
    join(tmpdir(), "pi-kit-web-worktree-timeout-group-"),
  );
  ...
  await chmod(script, 0o755);
  await expect(
    runWorktreeSetup(script, [childPidFile], directory, 2_000),
...
test("async managed cleanup yields before spawning Git verification", async () => {
  directory = await mkdtemp(
    join(tmpdir(), "pi-kit-web-worktree-async-cleanup-"),
  );
  ...
  if (exitCode !== 0)
    throw new Error(
      `Async cleanup probe failed: ${stderr.trim() || `exit ${exitCode}`}`,
    );
}, 5_000);
...
test("reused sessions inherit branch ownership when the original owner is deleted first", async () => {
  directory = await mkdtemp(
    join(tmpdir(), "pi-kit-web-worktree-owner-transfer-"),
  );
  const repository = join(directory, "repo");
  ...
  expect(
    inheritManagedBranchOwnership(rawReuse, [
      { ...owner, branchCreated: false },
    ]),
  ).toBe(rawReuse);
  ...
  await writeFile(
    ownerFile,
    `${JSON.stringify({ type: "session", cwd: owner.path })}\n`,
  );
  await writeFile(
    reuserFile,
    `${JSON.stringify({ type: "session", cwd: reuser.path })}\n`,
  );
  ...
  await rm(ownerFile);
  expect(hasOtherSessionInWorktree(sessions, reuserFile, reuser.path)).toBe(
    false,
  );
  ...
  expect(
    (
      await Bun.$`git -C ${repository} branch --list shared-owned`.text()
    ).trim(),
  ).toBe("");
});
...
test("worktree cleanup waits until the final saved session is deleted", async () => {
  ...
  await writeFile(
    current,
    `${JSON.stringify({ type: "session", cwd: worktree })}\n`,
  );
  await writeFile(
    other,
    `${JSON.stringify({ type: "session", cwd: join(worktree, "packages", "app") })}\n${"large transcript line\n".repeat(250_000)}`,
  );
...
test("worktree rollback removes the checkout and its branch", async () => {
  ...
  await expect(stat(result.path)).rejects.toThrow();
  expect(
    (await Bun.$`git -C ${repository} branch --list rollback-me`.text()).trim(),
  ).toBe("");
});

test("rollback removes operation-owned resources after the checkout branch changes", async () => {
  directory = await mkdtemp(
    join(tmpdir(), "pi-kit-web-worktree-rollback-ref-"),
  );
  const repository = join(directory, "repo");
  await Bun.$`git init -q ${repository}`;
  await Bun.$`git -C ${repository} config user.name test`;
  ...
  expect(
    (
      await Bun.$`git -C ${repository} branch --list rollback-original`.text()
    ).trim(),
  ).toBe("");
  expect(
    (
      await Bun.$`git -C ${repository} branch --list user-created`.text()
    ).trim(),
  ).toBe("user-created");
});
...
test("rollback preserves commits made after a managed branch was created", async () => {
  directory = await mkdtemp(
    join(tmpdir(), "pi-kit-web-worktree-rollback-advanced-"),
  );
  const repository = join(directory, "repo");
  await Bun.$`git init -q ${repository}`;
  await Bun.$`git -C ${repository} config user.name test`;
  ...
  const advancedHead = (
    await Bun.$`git -C ${result.path} rev-parse HEAD`.text()
  ).trim();
  rollbackWebWorktree(result);
  await expect(stat(result.path)).rejects.toThrow();
  expect(
    (
      await Bun.$`git -C ${repository} rev-parse rollback-advanced`.text()
    ).trim(),
  ).toBe(advancedHead);
});
...
test("rollback preserves a changed checkout with uncommitted files", async () => {
  directory = await mkdtemp(
    join(tmpdir(), "pi-kit-web-worktree-rollback-dirty-"),
  );
  const repository = join(directory, "repo");
  await Bun.$`git init -q ${repository}`;
  await Bun.$`git -C ${repository} config user.name test`;
  ...
  expect(() => rollbackWebWorktree(result)).toThrow(
    "uncommitted or untracked changes",
  );
  expect(await readFile(join(result.path, "uncommitted.txt"), "utf8")).toBe(
    "preserve me\n",
  );
  expect(
    (
      await Bun.$`git -C ${repository} branch --list rollback-dirty`.text()
    ).trim(),
  ).toBe("rollback-dirty");
});

test("worktree handling requires Git 2.36 or newer", () => {
  expect(() => validateGitVersion("git version 2.35.9")).toThrow(
    "Git 2.36.0 or newer is required",
  );
  expect(() => validateGitVersion("git version unknown")).toThrow(
  ...
  expect(() =>
    validateGitVersion("git version 2.39.5 (Apple Git-154)"),
  ).not.toThrow();
  expect(() =>
    validateGitVersion("git version 2.43.0.windows.1"),
  ).not.toThrow();
});
...
test("web worktree names reject traversal and nested paths", () => {
  ...
  expect(validateWorktreeName("fix-123_example.test")).toBe(
    "fix-123_example.test",
  );
  expect(validateLocalBranchName("tembo/cancel-builds")).toBe(
    "tembo/cancel-builds",
  );
  for (const value of [
    "bad..branch",
