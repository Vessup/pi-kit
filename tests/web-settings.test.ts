...
test("web settings writes preserve unrelated Pi and extension keys", async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({
      theme: "dark",
      web: { other: { enabled: true } },
      packageSetting: 42,
    }),
  );
  await writeWebTailscaleSettingFile(path, {
    enabled: true,
    httpsPort: 443,
    serviceName: "svc:pi-web",
  });
  expect(await readSettings(path)).toEqual({
    theme: "dark",
    web: {
      other: { enabled: true },
      tailscale: { enabled: true, httpsPort: 443, serviceName: "pi-web" },
    },
    packageSetting: 42,
  });
});
...
test("concurrent web settings updates remain valid and retain unrelated keys", async () => {
  ...
  await writeFile(
    path,
    JSON.stringify({ theme: "light", web: { other: "retained" } }),
  );
  await Promise.all([
    writeWebTailscaleSettingFile(path, {
      enabled: true,
      httpsPort: 8443,
      serviceName: "first",
    }),
  ...
  const result = (await readSettings(path)) as {
    theme?: string;
    web?: { other?: string; tailscale?: { httpsPort?: number } };
  };
...
test("malformed settings reject without leaking the cross-process lock", async () => {
  ...
  await expect(
    writeWebTailscaleSettingFile(path, { enabled: true, httpsPort: 443 }),
  ...
  expect(await readSettings(path)).toEqual({
    recovered: true,
    web: { tailscale: { enabled: false, httpsPort: 8443 } },
  });
});
