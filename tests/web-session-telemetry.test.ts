...
import { mergeWebSubagentUpdates, type WebSession, type WebSubagentUpdate, type WebUsage } from "../web/protocol.ts";
import { preserveSessionTelemetry, preserveSessionsTelemetry } from "../web/client/session-telemetry.ts";
...
test("session telemetry survives partial and transient zero-valued snapshots", () => {
	const previous = { ...session("one", 100), contextUsage: { tokens: 25, contextWindow: 1_000, percent: 2.5 } };
...
test("subagent transcript and streaming deltas merge without retransmitting prior content", () => {
	...
	expect(updated[0]?.transcript?.map((item) => item.text)).toEqual(["first", "done"]);
	...
	const reset = mergeWebSubagentUpdates(updated, [{
		...
		updatedAt: 4,
		transcriptReset: true,
		transcriptDelta: [{ timestamp: 3, role: "assistant", text: "retained tail" }],
		streamingTextReset: true,
		streamingTextDelta: "",
	}]);
	expect(reset[0]?.transcript?.map((item) => item.text)).toEqual(["retained tail"]);
