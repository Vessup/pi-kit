...
import { displaySessionStatus, hasActiveSessionWork } from "../web/client/session-status.ts";
...
test("offline sessions are labeled inactive regardless of their origin", () => {
	expect(displaySessionStatus({ source: "web", status: "offline" })).toBe("inactive");
	...
	expect(displaySessionStatus({ source: "web", status: "starting" })).toBe("starting");
});
...
test("active subagents keep an idle main session working and stoppable", () => {
	...
	for (const status of ["creating", "working", "terminating"] as const) {
		const session = { source: "web" as const, status: "idle" as const, subagents: [{ ...subagent, status }] };
