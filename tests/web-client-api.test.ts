...
import { commandHelloType, createSession, healthSupportsWorktreeRefs, sendSessionCommand, sessionCommandTimeout } from "../web/client/api.ts";
...
test("worktree capability is refreshed for each create request", async () => {
	...
	globalThis.fetch = (async (input: string | URL | Request) => {
		if (String(input) === "/api/health") {
			healthRequests += 1;
			return Response.json({ capabilities: { worktreeRefs: healthRequests === 1 } });
		}
		return Response.json({ session: { id: "session-1" } });
	}) as typeof fetch;
	try {
		await createSession({ cwd: "/repo", worktreeName: "topic", worktreeBranch: "topic" });
		await expect(createSession({ cwd: "/repo", worktreeName: "topic", worktreeBranch: "topic" })).rejects.toThrow("must be updated");
...
test("command capability is refreshed and a close during connect rejects immediately", async () => {
	...
	class MockWebSocket {
		...
		addEventListener(type: string, listener: (event: Event & { data?: string }) => void): void {
		...
		send(payload: string): void {
			...
			if (message.type === "client.hello" || message.type === "client.command_hello") {
				...
				if (MockWebSocket.closeOnHello) this.emit("close", { code: 1006, reason: "daemon replaced" } as CloseEvent);
				return;
			}
			queueMicrotask(() => this.emit("message", { data: JSON.stringify({ type: "server.response", requestId: message.requestId, success: true }) } as MessageEvent));
		}
	...
	let healthRequests = 0;
	globalThis.fetch = (async () => Response.json({ capabilities: { commandHello: healthRequests++ === 0 } })) as typeof fetch;
	Object.defineProperty(globalThis, "WebSocket", { configurable: true, value: MockWebSocket });
	Object.defineProperty(globalThis, "window", {
		configurable: true,
		value: { location: { origin: "http://localhost" }, setTimeout, clearTimeout },
	});
	try {
		await sendSessionCommand("session", { type: "abort" });
		await sendSessionCommand("session", { type: "abort" });
		expect(MockWebSocket.hellos).toEqual(["client.command_hello", "client.hello"]);

		MockWebSocket.closeOnHello = true;
		await expect(sendSessionCommand("session", { type: "abort" })).rejects.toThrow("Command socket closed (1006: daemon replaced)");
	} finally {
		globalThis.fetch = originalFetch;
