import { closeSync, existsSync, openSync, readSync, realpathSync, renameSync, statSync, unlinkSync, writeSync } from "node:fs";
...
import { parseWorktreeCommandArgs, WORKTREE_USAGE } from "../web/worktree-command.js";
import {
	WORKTREE_REPLACEMENT_ENTRY,
	type WorktreeSessionReplacement,
} from "../web/worktree-replacement.js";
export { WORKTREE_REPLACEMENT_ENTRY, replacementFromEntries } from "../web/worktree-replacement.js";
...
export const WorktreeToolParameters = Type.Object({
	name: Type.Optional(Type.String({ minLength: 1, description: "Safe directory name below <repo>/.pi/worktrees; defaults as the local branch only when branch is omitted" })),
	...
	branch: Type.Optional(Type.String({ minLength: 1, description: "Local branch to reuse or create; may contain slashes and defaults to name" })),
	startPoint: Type.Optional(Type.String({ minLength: 1, description: "Git ref or commit for a new local branch, such as origin/owner/topic; a remote-tracking ref configures upstream" })),
...
export function worktreeToolCommandArgs(input: WorktreeToolInput): string {
	...
	const branch = input.branch?.trim() ? validateLocalBranchName(input.branch) : undefined;
	...
	if (existing) {
		if (name || repository || branch || startPoint) throw new Error("Specify existing by itself, or name with optional repository, branch, and startPoint values");
	...
	return [
		...
		repository ? `--repo ${quoteCommandArgument(normalizeToolPath(repository))}` : undefined,
		branch ? `--branch ${quoteCommandArgument(branch)}` : undefined,
		startPoint ? `--start-point ${quoteCommandArgument(startPoint)}` : undefined,
...
type WorktreeSelection =
	| { mode: "create"; name: string; repository: string; branch?: string; startPoint?: string }
...
function verifiedSessionId(sessionFile: string): string {
	...
	try {
		...
		const header = JSON.parse(buffer.subarray(0, newline).toString("utf8")) as Record<string, unknown>;
		if (header.type !== "session" || typeof header.id !== "string") throw new Error("Session has no valid header ID");
		return header.id;
	} finally {
...
function assertSourceSessionIsNotOpen(sessionFile: string): void {
	...
	if (result.status !== 1) throw new Error(result.stderr.trim() || `Could not verify exclusive source-session ownership (lsof exited ${result.status})`);
}

function deleteVerifiedSourceSession(sessionFile: string, expectedSessionId: string, expected: SourceSessionSnapshot): void {
	...
	try {
		...
		if (current.size !== expected.size || current.mtimeMs !== expected.mtimeMs || current.ino !== expected.ino) {
			throw new Error("Source session changed after the replacement snapshot was created");
		}
		const actualSessionId = verifiedSessionId(tombstone);
		if (actualSessionId !== expectedSessionId) {
			throw new Error(`Source session identity changed (expected ${expectedSessionId}, found ${actualSessionId})`);
		}
		if (existsSync(sessionFile)) throw new Error("Source session path was recreated during deletion");
		const final = sourceSessionSnapshot(tombstone);
		if (final.size !== current.size || final.mtimeMs !== current.mtimeMs || final.ino !== current.ino) {
			throw new Error("Source session changed while deletion was being verified");
		}
		assertSourceSessionIsNotOpen(tombstone);
		if (existsSync(sessionFile)) throw new Error("Source session path was recreated during deletion");
		unlinkSync(tombstone);
	} catch (error) {
		if (!existsSync(sessionFile) && existsSync(tombstone)) renameSync(tombstone, sessionFile);
		throw error;
	}
}
...
export function createReplacementSession(
	...
	try {
		...
		if (cleanupError) throw new ReplacementSessionCleanupError(`${message}; could not remove failed replacement session: ${cleanupError}`);
...
async function runWorktreeCommandUnlocked(
	...
	const deleteSourceSession = dependencies.deleteSourceSession
		?? ((sessionFile: string) => deleteVerifiedSourceSession(sessionFile, previousSessionId, sourceSnapshot));
	...
	try {
		...
		const message = error instanceof Error ? error.message : String(error);
		const rollbackError = error instanceof ReplacementSessionCleanupError ? undefined : rollbackManagedTarget();
		throw new Error(rollbackError ? `${message}; managed worktree rollback failed: ${rollbackError}` : message);
	}
	...
	const previousManagedWorktree = managedWorktreeFromEntries(ctx.sessionManager.getEntries());
	const shouldRemovePreviousWorktree = process.env.PI_WEB_MANAGED !== "1" && Boolean(
		previousManagedWorktree && !hasOtherSessionInWorktree(
	...
	try {
		...
		const result = await ctx.switchSession(replacement.sessionFile, {
			withSession: async (next) => {
				...
				const rollback = async (message: string) => {
					...
					if (cleanupError) next.ui.notify(`Could not remove failed replacement session ${replacement.sessionFile}: ${cleanupError}. Managed worktree retained so that session CWD remains valid.`, "warning");
					const worktreeRollbackError = cleanupError ? undefined : rollbackManagedTarget();
					if (worktreeRollbackError) next.ui.notify(`Could not roll back managed worktree ${target.path}: ${worktreeRollbackError}`, "warning");
				...
				if (actualSessionId !== replacement.sessionId || !actualSessionFile) {
					await rollback(`Replacement session identity verification failed. Expected ${replacement.sessionId}, found ${actualSessionId}.`);
					return;
				}
				try {
					if (realpathSync(actualSessionFile) !== realpathSync(replacement.sessionFile)) {
						await rollback(`Replacement session file verification failed. Opened ${actualSessionFile}, not ${replacement.sessionFile}.`);
						return;
					}
				} catch (error) {
					await rollback(`Replacement session file verification failed: ${error instanceof Error ? error.message : String(error)}.`);
					return;
				}
				...
				(next.sessionManager as unknown as SessionManager).appendCustomEntry(WORKTREE_REPLACEMENT_ENTRY, replacedSession);
				if (process.env.PI_WEB_MANAGED !== "1") {
					try {
						deleteSourceSession(previousSessionFile);
					} catch (error) {
						await rollback(`Could not delete the source session: ${error instanceof Error ? error.message : String(error)}.`);
						return;
					}
				}
				...
				if (shouldRemovePreviousWorktree && previousManagedWorktree) {
					try {
						...
						if (cleanup.branchWarning) next.ui.notify(`Previous worktree removed, but branch ${previousManagedWorktree.branch} could not be deleted: ${cleanup.branchWarning}`, "warning");
					} catch (error) {
						next.ui.notify(`Source session deleted, but previous managed worktree cleanup failed: ${error instanceof Error ? error.message : String(error)}`, "warning");
					}
				}
				if (continuation) {
					try {
						...
						next.ui.notify(`Conversation moved successfully, but its continuation could not be sent: ${error instanceof Error ? error.message : String(error)}`, "warning");
					}
				}
			},
		});
		if (result.cancelled || !verifiedSessionId) {
			replacements().delete(replacement.sessionId);
			const cleanupError = removeReplacementSession(replacement.sessionFile);
			if (cleanupError) ctx.ui.notify(`Could not remove cancelled replacement session ${replacement.sessionFile}: ${cleanupError}. Managed worktree retained so that session CWD remains valid.`, "warning");
			if (!rollbackCompleted && !cleanupError) {
				const worktreeRollbackError = rollbackManagedTarget();
				if (worktreeRollbackError) ctx.ui.notify(`Could not roll back managed worktree ${target.path}: ${worktreeRollbackError}`, "warning");
			}
		...
		if (cleanupError || worktreeRollbackError) {
			throw new Error([
				message,
				cleanupError ? `could not remove failed replacement session: ${cleanupError}` : undefined,
				worktreeRollbackError ? `managed worktree rollback failed: ${worktreeRollbackError}` : undefined,
			].filter((value): value is string => value !== undefined).join("; "));
		}
		throw error;
	}
...
export default function worktreeExtension(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		name: "worktree",
		label: "Worktree",
		description: "Create a managed Git worktree with an independent directory name, local branch, and optional start ref/upstream, or enter an existing registered worktree; replace this Pi session and continue there.",
		promptSnippet: "Create or enter a managed Git worktree on the requested branch/ref, move this conversation there, and resume the task",
		promptGuidelines: [
	...
	pi.registerCommand("worktree", {
		description: "Create or enter a repository worktree on an optional branch/start ref and move this conversation into it",
