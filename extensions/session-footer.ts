...
export default function sessionFooter(pi: ExtensionAPI): void {
	...
	const unsubscribeContributions = pi.events.on(FOOTER_CONTRIBUTION_EVENT, (value) => {
		...
		ctx.ui.setFooter((tui, theme, footerData) => {
			...
			return {
				...
				render(width: number): string[] {
					...
					for (const contribution of contributions.values()) addUsage(totals, contribution.usage);
					...
					const contributionStatuses = Array.from(contributions.entries())
						.sort(([a], [b]) => a.localeCompare(b))
						.flatMap(([, contribution]) => (contribution.status ? [contribution.status] : []));
					...
					if (statuses.length > 0) {
						const statusLine = truncateToWidth(
							statuses.map((status) => sanitizeStatus(status.text)).filter(Boolean).join(" "),
							width,
							theme.fg("dim", "..."),
						);
						if (statuses.some((status) => status.selected)) {
							const padding = " ".repeat(Math.max(0, width - visibleWidth(statusLine)));
							lines.push(theme.bg("selectedBg", theme.fg("accent", statusLine + padding)));
						} else if (statusLine) {
