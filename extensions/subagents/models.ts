...
export function inheritedSubagentModel<
  T extends { provider: string; id: string },
>(current: T | undefined, runtimeModel: T | undefined): T | undefined {
...
export function subagentModelRuntime(
  ...
  if (!(runtime instanceof ModelRuntime)) {
    throw new Error(
      "The host model registry does not expose its canonical runtime",
    );
  }
  return runtime;
}
...
export function unavailableModelMessage(
  ...
  const inherit = current
    ? ` Omit model to inherit ${modelName(current)}.`
    : "";
