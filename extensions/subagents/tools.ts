...
const EffortSchema = stringEnum(THINKING_LEVELS, {
  description:
    "Reasoning effort. The selected model may clamp unsupported levels.",
});

const CreateParams = Type.Object({
  prompt: Type.String({
    description: "Complete task prompt for the new isolated subagent",
  }),
  name: Type.Optional(
    Type.String({
      description: "Short stable name used to address the subagent",
    }),
  ),
  model: Type.Optional(
    Type.String({
      description:
        "Exact provider/model-id or exact unambiguous model id. Omit to inherit the current model; never use a shortened family alias.",
    }),
  ),
  ...
  cwd: Type.Optional(
    Type.String({
      description:
        "Working directory, relative to the main session unless absolute",
    }),
  ),
});
...
const ReadParams = Type.Object({
  id: Type.Optional(
    Type.String({ description: "Subagent id. Omit to read all subagents." }),
  ),
  ...
  include_transcript: Type.Optional(
    Type.Boolean({
      description:
        "Include the full retained transcript instead of only latest output",
    }),
  ),
});
...
const SendParams = Type.Object({
  ...
  urgency: stringEnum(["normal", "urgent"] as const, {
    description:
      "urgent steers after the current tool batch; normal queues until the current run finishes",
  }),
});
...
const ConfigureParams = Type.Object({
  ...
  model: Type.Optional(
    Type.String({
      description:
        "Exact new provider/model-id or exact unambiguous model id. Omit to retain the current model; never use a shortened family alias.",
    }),
  ),
  effort: Type.Optional(EffortSchema),
});
...
const TerminateParams = Type.Object({
  id: Type.Optional(
    Type.String({
      description:
        "Subagent id. Omit with all=true to terminate every subagent.",
    }),
  ),
  all: Type.Optional(Type.Boolean({ description: "Terminate every subagent" })),
  remove: Type.Optional(
    Type.Boolean({
      description: "Also remove terminated records from the footer and manager",
    }),
  ),
});
...
export function registerSubagentTools(
  ...
  pi.registerTool({
    ...
    renderCall(args, theme) {
      ...
      return new Text(
        `${theme.fg("toolTitle", theme.bold("subagent_create"))}${name}${theme.fg("muted", model + effort)}\n${theme.fg("dim", truncateChars(args.prompt, 180))}`,
        0,
        0,
      );
    },
    renderResult(result, _options, theme) {
      ...
      return new Text(
        theme.fg(
          "toolOutput",
          text?.type === "text" ? text.text : "Created subagent",
        ),
        0,
        0,
      );
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal) {
      ...
      await manager.waitForUpdates(
        agents,
        params.wait_seconds ?? DEFAULT_READ_WAIT_SECONDS,
        signal,
      );
    ...
    renderCall(args, theme) {
      return new Text(
        theme.fg("toolTitle", theme.bold("subagent_read")) +
          theme.fg(
            "muted",
            ` ${args.id ?? "all"} · wait ${args.wait_seconds ?? DEFAULT_READ_WAIT_SECONDS}s`,
          ),
        0,
        0,
      );
  ...
  pi.registerTool({
    ...
    description:
      "Send an urgent steering message to a running subagent or queue a normal follow-up message for it.",
    promptSnippet:
      "Steer a subagent urgently or queue a normal follow-up instruction",
  ...
  pi.registerTool({
    ...
    description:
      "Change a retained subagent's model and/or reasoning effort. Changes apply to its next model request.",
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      return toolResult(
        manager,
        `${agent.id} now uses ${agent.model} at ${agent.effort} effort.`,
      );
    },
    renderCall(args, theme) {
      return new Text(
  ...
  pi.registerTool({
    ...
    description:
      "Abort one or all subagents, dispose their sessions, and optionally remove their retained transcript records.",
    ...
    async execute(_toolCallId, params) {
      if (params.all) {
        ...
        return toolResult(
          manager,
          "Terminated all subagents and released their session resources.",
        );
      }
      ...
      return toolResult(
        manager,
        `Terminated ${params.id} and released its session resources.`,
      );
    },
    renderCall(args, theme) {
      return new Text(
