...
const lockfile = createRequire(import.meta.url)("proper-lockfile") as { lock: LockSettingsFile };
...
export async function readWebTailscaleSetting(): Promise<TailscaleWebSettings> {
	try {
		...
		const serviceName = typeof tailscale.serviceName === "string"
...
export async function writeWebTailscaleSettingFile(
	...
	const release = await io.lock(settingsPath, { realpath: false, retries: { retries: 9, minTimeout: 20, maxTimeout: 20 } });
	try {
		let root: Record<string, unknown> = {};
		try {
			const parsed: unknown = JSON.parse(await io.readFile(settingsPath, "utf8"));
			if (isRecord(parsed)) root = parsed;
		} catch (error) {
			if (error && typeof error === "object" && "code" in error && error.code === "ENOENT") {
				// First global setting write.
			} else {
				throw new Error(`Could not read ${settingsPath}: ${error instanceof Error ? error.message : String(error)}`);
			}
		}
		...
		web.tailscale = {
			...
			...(setting.serviceName ? { serviceName: setting.serviceName.replace(/^svc:/, "") } : {}),
		};
		root.web = web;
		const tempPath = join(settingsDir, `.settings.${process.pid}.${io.randomUUID()}.tmp`);
		try {
...
export async function writeWebTailscaleSetting(setting: TailscaleWebSettings): Promise<void> {
