...
export default function deleteSessionExtension(pi: ExtensionAPI) {
  pi.registerCommand("delete-session", {
    ...
    handler: async (_args, ctx) => {
      ...
      if (!sessionFile) {
        ctx.ui.notify(
          "This session is not persisted, so there is no session file to delete.",
          "warning",
        );
        return;
      }

      const managedWorktree = managedWorktreeFromEntries(
      ...
      try {
        ...
        ctx.ui.notify(
          `Could not delete the current session: ${message}`,
          "error",
        );
        return;
      }
      ...
      if (shouldRemoveWorktreeNow && managedWorktree) {
        try {
          ...
          if (result.branchWarning)
            ctx.ui.notify(
              `Worktree removed, but branch ${managedWorktree.branch} could not be deleted: ${result.branchWarning}`,
              "warning",
            );
        } catch (error) {
          ctx.ui.notify(
            `Session deleted, but managed worktree cleanup failed: ${error instanceof Error ? error.message : String(error)}`,
            "error",
          );
        }
      }
